...
st.write("직업은 무엇인가요?")
job = st.text_input("직업을 입력해주세요...")

st.write("외모는 어떤가요?")
face = st.text_input("외모를 묘사해주세요...")

st.write("성격은 어떤가요?")
emotion = st.text_input("성격을 묘사해주세요...")

st.write("특기가 무엇인가요?")
special = st.text_input("특기를 설명해주세요...")

st.write("캐릭터의 지능을 0에서 10사이의 숫자로 매긴다면 몇인가요?")
int = 0
while int >= 0 and int <= 10:
    int = st.number_input("지능을 설정해주세요...")
    if not int >= 0 and int <= 10:
        st.write("0에서 10사이의 숫자로 설정해주세요.")

st.write("캐릭터의 마력을 0에서 10사이의 숫자로 매긴다면 몇인가요?")
mp = 0
while mp >= 0 and mp <= 10:
    mp = st.number_input("마력을 설정해주세요...")
    if not mp >= 0 and mp <= 10:
        st.write("0에서 10사이의 숫자로 설정해주세요.")

st.write("캐릭터의 관찰력을 0에서 10사이의 숫자로 매긴다면 몇인가요?")
sight = 0
while sight >= 0 and sight <= 10:
    sight = st.number_input("관찰력을 설정해주세요...")
    if not sight >= 0 and sight <= 10:
        st.write("0에서 10사이의 숫자로 설정해주세요.")

st.write("캐릭터의 민첩을 0에서 10사이의 숫자로 매긴다면 몇인가요?")
dex = 0
while dex >= 0 and dex <= 10:
    dex = st.number_input("민첩을 설정해주세요...")
    if not dex >= 0 and dex <= 10:
        st.write("0에서 10사이의 숫자로 설정해주세요.")
